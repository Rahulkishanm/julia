        ...
        bool *needStackRestore)
{
    ...
    if (!jvinfo.ispointer()) {
        tbaa_decorate(jvinfo.tbaa, ctx.builder.CreateAlignedStore(emit_unbox(ctx, to, jvinfo, jlto), slot, align));
    }
    else {
        emit_memcpy(ctx, slot, jvinfo.tbaa, jvinfo, jl_datatype_size(jlto), align);
    }
    return slot;
}
